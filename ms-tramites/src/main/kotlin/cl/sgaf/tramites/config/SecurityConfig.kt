...
import cl.sgaf.tramites.domain.Rol
import cl.sgaf.tramites.dto.APIResponse
import cl.sgaf.tramites.dto.ErrorResponse
...
// Define Rol locally inside config or import it? Let's check where Rol is.
// Ah, in ms-tramites we don't have Rol enum defined in ms-tramites/domain. Let's define Rol enum in ms-tramites/domain!
// Wait! Yes, ms-tramites also needs Rol enum to distinguish roles. Let's make sure we define Rol enum in domain layer.
...
class HeaderAuthFilter(private val objectMapper: ObjectMapper) : OncePerRequestFilter() {
    override fun doFilterInternal(
        ...
        val userAduanaHeader = request.getHeader("X-User-Aduana")
        ...
        val currentUser = CurrentUser(id = userId, rol = rol, aduana = userAduanaHeader)
        ...
        http
            ...
            .authorizeHttpRequests { auth ->
                auth.requestMatchers(
                    "/api/tramites/swagger-ui/**",
                    "/api/tramites/swagger-ui.html",
                    "/v3/api-docs/**",
                    "/actuator/**"
                ).permitAll()
